TriggerEvent('es:addGroupCommand', 'tp', 'admin', function(source, args, user)
	...
	if x and y and z then
		...
		TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Coordonnees invalides!")
	end
	...
	if tonumber(args[1]) and args[2] and tonumber(args[3]) then
		...
		if xPlayer then
			if ESX.DoesJobExist(args[2], args[3]) then
				...
				TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Job inexistant' } })
			end
		...
		TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Usage invalide' } })
	end
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Permissions insuffisantes' } })
...
TriggerEvent('es:addGroupCommand', 'setmoney', 'superadmin', function(source, args, user)
	...
	if target and money_type and money_amount and xPlayer ~= nil then
		...
		TriggerClientEvent('chatMessage', _source, "SYSTEM", {255, 0, 0}, "Arguments invalides")
		return
	end
...
TriggerEvent('es:addGroupCommand', 'c', 'user', function(source, args, user)
	TriggerClientEvent('chat:clear', source)
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Permissions insuffisantes' } })
end)

TriggerEvent('es:addGroupCommand', 'call', 'admin', function(source, args, user)
